...
static void refill_hw_fifo(struct eh_device *eh_dev)
{
	...
	spin_lock(&fifo->lock);
	cookie = list_first_entry_or_null(&fifo->head, typeof(*cookie), list);
	if (cookie) {
		do {
			/*
			 * Take the cookie off the list since it can't be
			 * touched once it's passed onto the compression thread.
			 */
			list_del(&cookie->list);
			spin_unlock(&fifo->lock);

			/* Attempt to pass the cookie onto the hardware fifo */
			ret = request_to_hw_fifo(eh_dev, cookie, false);

			spin_lock(&fifo->lock);
			if (ret) {
				/* Add the cookie back to the front */
				list_add(&cookie->list, &fifo->head);
				break;
			}
		} while ((cookie = list_first_entry_or_null(&fifo->head,
							    typeof(*cookie),
							    list)));
		spin_unlock(&fifo->lock);
		return;
...
static irqreturn_t eh_compress_irq(int irq, void *data)
{
	...
	/*
	 * Clear the interrupt status and check the completed index afterwards
	 * to know for certain that there aren't any new compressions finished.
	 * Otherwise, this could end up clearing the interrupt status for an
	 * actual compression completion without waking the compression thread.
	 */
	eh_clear_cintr_status(eh_dev);

	/* Nothing to do if the compression thread isn't waiting */
	if (!swait_active(&eh_dev->cirq_wq))
		return IRQ_HANDLED;

	/*
	 * Check that this isn't an interrupt for a stale completion. If it is,
	 * then the compression thread won't break out of its sleep since it'll
	 * see that there aren't actually any new compression requests finished.
	 */
	if (fifo_next_complete_index(eh_dev) == eh_dev->complete_index)
		return IRQ_HANDLED;

	/* Mask the interrupt and then wake up the compression thread */
...
static unsigned int eh_wait_next_index(struct eh_device *eh_dev, unsigned int i)
{
	...
	if ((end = fifo_next_complete_index(eh_dev)) != i) {
		...
		do {
			schedule();
			prepare_to_swait_exclusive(&eh_dev->cirq_wq, &wait,
						   TASK_IDLE);
